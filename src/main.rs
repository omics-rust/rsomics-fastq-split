...
fn build_config(cli: &Cli) -> Result<SplitConfig> {
    let mode = match (cli.split, cli.split_by_lines) {
        (Some(_), Some(_)) => {
    ...
    Ok(SplitConfig {
        mode,
        digits: cli.split_prefix_digits,
    })
}

fn pipeline(cli: &Cli) -> Result<SplitReport> {
